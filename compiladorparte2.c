...
TInfoAtomo obter_atomo() {
    ...
    //if (info.atomo == IDENTIFICADOR) printf(" | %s", info.lexema);
    //else if (info.atomo == INTCONST) printf(" | %d", info.atributo_decimal);
    //else if (info.atomo == CHARCONST) printf(" | %c", info.atributo_caractere);
    //printf("\n");

    return info;
}

// Reconhece se for identificadores ou palavras reservadas
TInfoAtomo reconhece_id_ou_palavra() {
